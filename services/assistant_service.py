...
def crear_asistente(
    ...
    db = SessionLocal()
    try:
        if _proyecto_del_usuario(db, project_id, owner_id) is None:
            return None
...
def obtener_asistentes(project_id: int, owner_id: int):
    ...
    try:
        ...
        return (
            db.query(Assistant)
            .filter(Assistant.project_id == project_id)
...
def obtener_asistente(assistant_id: int, owner_id: int):
    db = SessionLocal()
    try:
        return (
            db.query(Assistant)
...
def eliminar_asistente(assistant_id: int, owner_id: int):
    db = SessionLocal()
    try:
        asistente = (
            db.query(Assistant)
